...
contract MedicalRecords {
        ...
        returns (string memory) 
    {
        ...
        // Check if caller is the patient themselves
        if (patients[patientId].walletAddress == msg.sender) {
            uint256 length = patients[patientId].privateCIDs.length;
            require(length > 0, "No records found");
            return patients[patientId].privateCIDs[length - 1];  // Patient can view their own latest record
        }
        
        // Otherwise, check if caller is a doctor with view access
        string memory doctorId = walletToDoctorId[msg.sender];
        require(bytes(doctorId).length > 0, "Doctor not found");
        require(doctors[doctorId].isRegistered, "Doctor not registered");
        ...
        returns (string[] memory) 
    {
        ...
        // Check if caller is the patient themselves
        if (patients[patientId].walletAddress == msg.sender) {
            return patients[patientId].privateCIDs;  // Patient can view their own records
        }
